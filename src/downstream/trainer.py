...
class DownstreamTrainer:
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader) -> dict[str, float]:
        """Train and evaluate by epoch; append metrics to markdown output."""
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            ...
            self.logger.info(
                "epoch=%d mode=%s train_loss=%.4f val_acc=%.4f val_macro_f1=%.4f time=%.2fs",
                epoch,
                self.mode,
                train_loss,
                metrics["accuracy"],
    ...
    def evaluate(self, dataloader: DataLoader) -> dict[str, float]:
        """Evaluate downstream classifier and return scalar metrics."""
        self.model.eval()
        y_true: list[int] = []
        y_pred: list[int] = []
