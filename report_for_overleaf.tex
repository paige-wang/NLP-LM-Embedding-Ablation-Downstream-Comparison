...
In the second part, embedding initialization is studied through a complete $3 \times 3$ grid under a unified compute budget (10 epochs, max 400 steps per epoch, batch size 128). Across RNN, LSTM, and Transformer backbones, fixed self-trained Word2Vec embeddings achieve the lowest validation perplexity, while GloVe still covers 91.99\% of the joint vocabulary.
...
Taken together, the results show that fixed-budget LM perplexity and downstream transfer value are not identical: corpus-matched fixed embeddings help early LM optimization most, high-coverage GloVe gives the strongest frozen downstream baseline, and full parameter updates remain necessary for the best task-specific adaptation.
\end{abstract}
...
\Cref{tab:qualitative_samples} gives greedy continuations from a shared prompt. The ranking matches the perplexity results: the Trigram is locally repetitive, the RNN is syntactically plausible but generic, the LSTM introduces more concrete content, and the Transformer is the only model that sustains topic-level coherence across the full continuation.

\begin{table}[htbp]
\centering
\caption{Greedy continuations from the shared prompt ``The historical city of''.}
\label{tab:qualitative_samples}
\scriptsize
\renewcommand{\arraystretch}{1.03}
\begin{tabularx}{\linewidth}{@{}lX@{}}
\toprule
\textbf{Model} & \textbf{Continuation} \\
\midrule
Trigram & a the and in of the \ldots \\
RNN & the world is a significant and the first \ldots \\
LSTM & London was built in the early 19th century \ldots \\
Transformer & Rome remains a central hub for classical architecture and Mediterranean trade \ldots \\
\bottomrule
\end{tabularx}
\end{table}

This qualitative comparison matters because perplexity alone can understate \emph{how} a model fails.
...
\subsection{Budget-Controlled $3 \times 3$ Design}
\label{sec:part2_scope}

Part~II studies a full $3 \times 3$ ablation over architecture and embedding type. The three model families are \textbf{RNN}, \textbf{LSTM}, and \textbf{Transformer}; the three embedding settings are \textbf{trainable}, \textbf{fixed self-trained} (Word2Vec on the project corpus), and \textbf{fixed pretrained} (GloVe-6B).

To keep all nine runs directly comparable under limited compute, every configuration is trained with the same budget: 10 epochs, at most 400 optimization steps per epoch, and batch size 128 on the same GPU. The resulting comparison should therefore be read as an \emph{equal-compute early-convergence study}, not as a claim about asymptotic performance after unrestricted training.

\subsection{Corpus-Matched Fixed Embeddings Win Under a Fixed LM Budget}
\label{sec:part2_coverage}

\Cref{tab:part2_grid,fig:part2_budget} summarize the Part~II results. A consistent pattern emerges across all three backbones: the \emph{fixed self-trained} embedding matrix gives the best final validation perplexity under the shared budget. Relative to jointly trainable embeddings, Word2Vec initialization reduces final validation perplexity by 9.5\% for the RNN, 9.6\% for the LSTM, and 18.3\% for the Transformer. Relative to fixed GloVe, the gains are 11.3\%, 24.6\%, and 6.8\%, respectively.

\begin{table}[htbp]
\centering
\caption{Part~II results under a unified compute budget. Entries are final validation perplexity after 10 epochs with at most 400 steps per epoch; lower is better.}
\label{tab:part2_grid}
\small
\renewcommand{\arraystretch}{1.06}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Model} & \textbf{Trainable} & \textbf{Fixed Self} & \textbf{GloVe} \\
\midrule
RNN         & 183.27 & \textbf{165.95} & 187.01 \\
LSTM        & 336.58 & \textbf{304.21} & 403.49 \\
Transformer & 213.45 & \textbf{174.43} & 187.11 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{subfigure}[t]{0.31\linewidth}
    ...
    \addplot[fill=cGreen!75, draw=cGreen!90!black] coordinates {(Covered,91.99) (OOV,8.01)};
    \end{axis}
    \end{tikzpicture}
    \caption{GloVe coverage in the joint vocabulary.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.66\linewidth}
    ...
    \begin{axis}[
        ...
        ylabel={Final Val PPL},
        symbolic x coords={RNN,LSTM,Transformer},
        xtick=data,
        tick label style={font=\scriptsize},
        ylabel style={font=\scriptsize},
        legend style={
    ...
    \addplot[fill=cBlue!75, draw=cBlue!90] coordinates {(RNN,183.27) (LSTM,336.58) (Transformer,213.45)};
    \addplot[fill=cOrange!80, draw=cOrange!95] coordinates {(RNN,165.95) (LSTM,304.21) (Transformer,174.43)};
    \addplot[fill=cGreen!75, draw=cGreen!90!black] coordinates {(RNN,187.01) (LSTM,403.49) (Transformer,187.11)};
    \legend{Trainable,Fixed self,GloVe}
    \end{axis}
    \end{tikzpicture}
    \caption{$3 \times 3$ language-model comparison under equal compute.}
\end{subfigure}
\caption{Part~II results under a unified compute budget. High lexical coverage is preserved ($91.99\%$), but the lowest validation perplexity comes from corpus-matched fixed self-trained embeddings rather than GloVe.}
\label{fig:part2_budget}
\end{figure}

The runtime side of the comparison is controlled as well. Mean epoch time stays within 10.56--13.33 seconds across all nine runs, so the ranking in \Cref{tab:part2_grid} is not explained by unequal compute. Under this budget, fixing a self-trained embedding matrix stabilizes optimization early and lets each backbone spend its limited steps on sequence modeling rather than first building token geometry from scratch.

The early epochs point in the same direction. For the RNN, fixed self-trained embeddings reach validation perplexity 258.08 by epoch~3 versus 298.91 for trainable embeddings; for the Transformer, the corresponding values are 268.22 versus 339.15. The LSTM gap is smaller early on, but the same configuration still finishes best at epoch~10.

\subsection{Coverage Still Matters, But for a Different Objective}

GloVe still covers 91.99\% of the joint vocabulary, which is essential for Part~III. However, high coverage alone does not guarantee the best \emph{language-model} perplexity under a short budget. The LM objective rewards embeddings that align tightly with the token distribution and local co-occurrence structure of WikiText-2; on that criterion, corpus-trained Word2Vec has an advantage over broader-domain GloVe. This effect is especially clear for the LSTM, where fixed self-trained embeddings outperform fixed GloVe by nearly one quarter in validation perplexity.

This distinction reconciles Parts~II and~III rather than contradicting them. Part~II measures how efficiently a backbone can improve next-token prediction under equal compute. Part~III instead measures what information a frozen linear head can access for intent separation on short SNIPS utterances. Public GloVe vectors encode broad lexical semantics that are highly useful once the vocabulary exposes the relevant intent words, whereas self-trained embeddings are better matched to the WikiText-2 language-model objective itself. The stronger embedding source therefore depends on \emph{which downstream question is being asked}: early LM optimization favors corpus-matched fixed embeddings, while frozen intent transfer favors high-coverage public semantics.
...
The fine-tuned curve also reaches its plateau early rather than requiring many extra epochs to overtake the frozen baselines. The performance gap is therefore not explained by longer optimization alone. Once gradients flow into the Transformer stack, the model can reshape attention patterns and intermediate features around SNIPS-specific evidence, whereas the frozen settings can only search for a better linear separator on top of fixed representations.
...
Taken together, the three experimental parts separate backbone quality, embedding quality, and adaptation quality. Part~I identifies the Transformer as the only backbone strong enough in perplexity and inference efficiency to justify transfer. Part~II shows that, under equal compute, corpus-matched fixed self-trained embeddings give the lowest validation perplexity across RNN, LSTM, and Transformer, while Part~III shows that high-coverage GloVe provides the strongest frozen downstream baseline on SNIPS. The representation that is best for early next-token optimization is therefore not automatically the one that is most linearly accessible for intent classification. The best overall system still comes from full fine-tuning: although GloVe provides a strong lexical prior, full fine-tuning allows the Transformer's deeper attention heads to undergo \emph{structural adaptation} to SNIPS. Part~II should therefore be read as a controlled-compute comparison rather than a full-convergence study, but within that regime the overall picture is consistent: strong language modeling identifies the right backbone, corpus-matched fixed embeddings help LM optimization most, high lexical coverage makes frozen GloVe useful on SNIPS, and full parameter updates remain necessary when the encoder must reorganize evidence across tokens.
